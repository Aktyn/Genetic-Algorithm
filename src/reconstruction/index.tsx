...
export default class Reconstruction extends React.Component<
  any,
  ReconstructionState
> {
  private source_images: (HTMLImageElement | null)[] = new Array(
    example_images.length
  ).fill(null);
  ...
  async onLoad() {
    ...
    for (let i = 0; i < data.data.length / 4; i++) {
      ...
      //grayscale
      let cLinear =
        0.2126 * (data.data[i * 4] / 255) +
        0.7152 * (data.data[i * 4 + 1] / 255) +
        0.0722 * (data.data[i * 4 + 2] / 255);

      if (cLinear <= 0.0031308) this.source_data[i] = 12.92 * cLinear;
      else this.source_data[i] = 1.055 * cLinear - 0.055;
      data.data[i * 4] =
        data.data[i * 4 + 1] =
        data.data[i * 4 + 2] =
          (this.source_data[i] * 256) | 0;
  ...
  switchTraining() {
    if (this.state.training) this.training_id++;
  ...
  epoch() {
    if (!this.source_data || this.preview_ctxs.length < POPULATION)
      throw new Error(
        "Cannot star evolution cause incorrect data initialization"
      );
    if (!this.evolution) throw new Error("evolution is not initialized");
    ...
    for (let i = 0; i < POPULATION; i++) {
      ...
      for (
        let j = result_heap_index;
        j < result_heap_index + BUFFER_SIZE;
        j += VALUES_PER_ELEMENT
      ) {
      ...
      //calculate score
      let preview_data = this.preview_ctxs[i].getImageData(
        0,
        0,
        INPUT_W,
        INPUT_H
      ...
      for (let j = offset; j < this.source_data.length; j += step) {
        ...
        score +=
          1.0 - Math.abs(preview_data.data[j * 4] / 255 - this.source_data[j]);
      }
    ...
    console.log(
      `generation: ${this.evolution.getGeneration()}, best score: ${
        this.evolution.getBestScore() | 0
      }, ${
    ...
    return (
      <div>
        <div style={{ height: "300px" }}>
          {example_images.map((img_src, index) => {
            return (
              <img
                key={index}
                ref={(el) => (this.source_images[index] = el)}
                src={img_src}
                style={{
                  maxHeight: "100%",
                  maxWidth: "100%",
        ...
        <div style={{ padding: "10px 0px" }}>
          <button onClick={this.switchTraining.bind(this)}>
        ...
        >
          ...
          <div
            ref={(el) => (this.preview_container = el)}
